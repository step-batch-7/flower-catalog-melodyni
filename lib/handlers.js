...
const appendComments = function (commentBoxes, comment) {
  commentBoxes += loadTemplate('commentBox.html', comment);
  return commentBoxes;
};

const collectComments = function (comments) {
  const commentBoxes = comments.reduce(appendComments, '');
  return loadTemplate('/../public/guestBook.html', { comments: commentBoxes });
};
